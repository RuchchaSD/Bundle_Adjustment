// OptimizationAlgorithmLevenberg.cpp
...
void OptimizationAlgorithmLevenberg::Optimize(int maxIterations) {
    ...
    if (p->DebugMode && p->verbosityLevel > 3)
        parameters->printParameterVector();
    ...
    while (!stop && current_iteration < maxIterations) {
        ...
        while (repeat_attempt < max_repeats) {
            ...
            numerator = (std::pow(prev_residuals_norm, 2) - std::pow(current_residuals_norm, 2));
            ...
            if (rho >= 0) {
                ...
                parameters->restore();
                current_residuals_norm = prev_residuals_norm;
...
void OptimizationAlgorithmLevenberg::initialize() {
    if (p == nullptr)
        std::cerr << "OptimizationAlgorithmLevenberg::initialize: properties not set" << std::endl;
